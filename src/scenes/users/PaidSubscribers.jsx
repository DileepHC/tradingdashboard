...
import { Edit, Trash2, SlidersHorizontal, Download, DollarSign, PlusCircle, X } from 'lucide-react';
import { dummyNames } from '../../data/mockData'; // Corrected import path for dummyNames
...
function PaidSubscribers({ data = [] }) {
  ...
  const [visibleColumns, setVisibleColumns] = useState({
    ...
    phoneEmail: true, // Re-enabled Phone/Email column
  ...
  const handleExport = (format) => {
    ...
    if (format === 'xls') {
      ...
      sortedAndFilteredData.forEach(user => {
        ...
        fileContent += `Remaining Days: ${user.remainingDays}\n`; // Include remaining days
  ...
  const handleFormSubmit = (e) => {
    ...
    // Get a random dummy name from the mock data
    ...
    const newSubscriber = {
      // Generate a 6-character alphanumeric User ID for new subscribers
      userId: editSubscriber ? editSubscriber.userId : generateAlphanumericId(6),
      tradingViewId,
      name: randomName, // Assign the random dummy name
      phoneEmail,
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-8 bg-bg-base text-text-base">
      ...
      {/* Applied overflow-x-auto to the kpi-card-custom wrapper */}
      <div className="kpi-card-custom overflow-x-auto">
          ...
          <div className="flex space-x-2 relative">
            ...
            {isColumnToggleOpen && (
              <div
                ...
                className="column-toggle-dropdown-custom absolute top-full right-0 mt-2 bg-card-bg border border-border-base rounded-lg shadow-strong z-10 p-4 min-w-[150px]"
              >
        ...
        {/* Removed min-w-max here, as explicit column widths and table-layout: fixed will manage width */}
        <table className="table-custom divide-y divide-border-base">
          <thead className="bg-bg-base">
            <tr>
              {/* Conditionally render table headers based on column visibility and set explicit widths */}
          ...
          <tbody className="divide-y divide-border-base">
            ...
            {sortedAndFilteredData.map((user, index) => (
              <tr key={user.userId || index}>
                {visibleColumns.userId && <td className="px-6 py-4 whitespace-nowrap text-sm text-text-base table-sticky-col">{user.userId}</td>}
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  // No maxLength for phoneEmail as it handles both 10-digit phone and variable-length email
                  required
                />
